...
import { Shield, CheckCircle2, Phone, Mail, Globe, Printer, Download } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 p-4 bg-white rounded-2xl border border-slate-200 shadow-sm print:hidden">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-bold text-slate-800">
            Official Electronic Policy Certificate: {policy.policy_number}
          </span>
        </div>
        ...
        <div className="flex items-center gap-3">
          <button
            onClick={handlePrint}
            className="inline-flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold text-slate-700 bg-slate-100 hover:bg-slate-200 transition-all"
          >
          ...
          <button
            onClick={handlePrint}
            className="inline-flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold text-white bg-brand-600 hover:bg-brand-700 transition-all shadow-sm"
          >
      ...
      <div 
        ...
        className="bg-white p-8 sm:p-12 rounded-3xl border-2 border-slate-200 shadow-xl max-w-4xl mx-auto text-slate-800 space-y-8 print:p-0 print:border-none print:shadow-none"
      >
        {/* Certificate Header */}
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 pb-6 border-b-2 border-slate-900">
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 rounded-2xl bg-brand-600 flex items-center justify-center text-white font-black shadow-md">
              <Shield className="w-7 h-7" />
            </div>
            <div>
              <h2 className="text-2xl font-black tracking-tight text-slate-900">EKTA TRAVELING</h2>
              <p className="text-xs font-semibold text-brand-600 tracking-wider uppercase">
          ...
          <div className="text-right sm:border-l sm:pl-6 border-slate-200">
            <div className="text-xs text-slate-500 font-bold uppercase tracking-wider">Policy Certificate No.</div>
            <div className="text-xl font-mono font-black text-brand-700">{policy.policy_number}</div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 p-4 rounded-2xl bg-slate-50/70 border border-slate-200 text-xs">
          <div>
            <span className="text-slate-400 block font-semibold mb-0.5">Coverage Territory</span>
            <span className="text-slate-900 font-bold text-sm">{policy.destination_name}</span>
          </div>

          <div>
            <span className="text-slate-400 block font-semibold mb-0.5">Insurance Plan</span>
            <span className="text-brand-600 font-bold text-sm">{policy.plan_name}</span>
          </div>

          <div>
            <span className="text-slate-400 block font-semibold mb-0.5">Valid From</span>
            <span className="text-slate-900 font-bold text-sm">{policy.start_date}</span>
          </div>

          <div>
            <span className="text-slate-400 block font-semibold mb-0.5">Valid Until</span>
              ...
              <tbody className="divide-y divide-slate-100">
                {travelers.map((t, i) => (
                  <tr key={i} className="hover:bg-slate-50/50">
                    ...
                    <td className="p-3 font-bold text-emerald-700">{policy.medical_limit}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
            <div className="p-3 rounded-xl border border-slate-200 flex justify-between items-center">
            ...
            <div className="p-3 rounded-xl border border-slate-200 flex justify-between items-center">
              ...
              <span className="font-bold text-emerald-600">€0 (Zero Deductible)</span>
            </div>
          </div>
        </div>
        ...
        <div className="p-6 rounded-2xl bg-gradient-to-r from-slate-900 to-brand-950 text-white space-y-3">
          ...
          <div className="flex flex-wrap gap-6 pt-2 text-xs font-mono font-bold">
            ...
            <div>
              ...
              <span className="text-white">{assistance.email || 'support@ektatraveling.com'}</span>
            </div>
          </div>
        </div>
        ...
        <div className="pt-6 border-t border-slate-200 flex flex-col sm:flex-row items-center justify-between gap-6 text-xs text-slate-500">
          <div className="flex items-center gap-4">
            ...
            <div className="w-16 h-16 bg-slate-900 rounded-xl p-1.5 flex flex-col justify-between flex-shrink-0">
              ...
              <div className="flex justify-between">
                <div className="w-3.5 h-3.5 bg-white rounded-sm" />
                <div className="w-3.5 h-3.5 bg-brand-500 rounded-sm" />
              </div>
            </div>
            <div>
              <div className="font-bold text-slate-900">Scan to Verify Authenticity</div>
              <div className="text-[11px] text-slate-500">
                Online verification URL: /api/v1/policies/validate/{policy.policy_number}
              </div>
          ...
          <div className="text-center sm:text-right">
            <div className="font-serif italic text-base font-bold text-slate-800">EKTA Traveling Digital Underwriting</div>
            <div className="text-[10px] uppercase tracking-wider text-slate-400">Authorized Electronic Signature</div>
          </div>
        </div>
      </div>
    </div>
