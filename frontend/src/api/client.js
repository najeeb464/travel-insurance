...
api.interceptors.response.use(
  ...
  async (error) => {
    ...
    if (error.response?.status === 401 && !originalRequest._retry) {
      ...
      const refreshToken = localStorage.getItem('tavara_refresh_token') || localStorage.getItem('ekta_refresh_token');
      if (refreshToken) {
        try {
          ...
          localStorage.setItem('tavara_access_token', newAccess);
